...
import { InteriorOptions, ModelFallback, SceneBoundary, useSceneQuality, useSceneReady } from "./ApartmentDollhouse";
...
function LookControls({ design, room, reset }: {
    ...
    useEffect(() => {
        ...
        camera.lookAt((r.x + r.w * .5) / 1000, storeyOffset + 1.25, (r.z + 250) / 1000);
...
export default function Tour360Viewer({ apartment = apartments[0] }: {
    ...
    const [room, setRoom] = useState("oh"), [tourLevel, setTourLevel] = useState(1), [style, setStyle] = useState<InteriorStyle>("nordic"), [furnished, setFurnished] = useState(true), [minimap, setMinimap] = useState(false), [reset, setReset] = useState(0), [notice, setNotice] = useState("");
    ...
    <div ref={ref} className="tour-viewport relative h-[min(58svh,430px)] min-h-[340px] bg-[#e1e7e5] sm:h-[650px]">{unavailable && <ModelFallback apartment={apartment}/>}<SceneBoundary fallback={<ModelFallback apartment={apartment}/>}>
      {ready && <Canvas shadows dpr={compactScene ? [1, 1.25] : [1, 1.75]} camera={{ position: cameraSpot(design, "oh"), fov: 72, near: .03, far: 120 }} gl={{ antialias: true, powerPreference: "high-performance" }} onCreated={({ gl }) => { gl.shadowMap.type = THREE.PCFSoftShadowMap; gl.toneMapping = THREE.ACESFilmicToneMapping; gl.toneMappingExposure = 1.08; }}><color attach="background" args={["#c6dce4"]}/><hemisphereLight args={["#f3f6ff", "#b9aa8f", 2.5]}/><directionalLight position={[-8, 14, -10]} intensity={3.2} castShadow shadow-mapSize={compactScene ? [1024, 1024] : [2048, 2048]} shadow-camera-left={-18} shadow-camera-right={18} shadow-camera-top={18} shadow-camera-bottom={-18} shadow-bias={-.0003}/><ArchitecturalModel design={design} style={style} furnished={furnished} interior/><mesh position={[design.width / 2000, -.3, design.depth / 2000]} rotation={[-Math.PI / 2, 0, 0]} receiveShadow><planeGeometry args={[160, 160]}/><meshStandardMaterial color="#96a788" roughness={1}/></mesh><LookControls design={design} room={room} reset={reset}/><Portals design={design} room={room} select={setRoom}/></Canvas>}
