...
export default function ApartmentDollhouse({ apartment = apartments[0] }: {
    ...
    <div ref={ref} className="relative h-[480px] sm:h-[660px]" role="group" aria-label={`${apartment.id} 3D`}>{unavailable && <ModelFallback apartment={apartment}/>}<SceneBoundary fallback={<ModelFallback apartment={apartment}/>}>
      {ready && <Canvas shadows dpr={[1, 2]} camera={{ position: [w / 2 + extent * .82, extent * 1.1, d / 2 + extent * .92], fov: 42, near: .1, far: 120 }} gl={{ antialias: true }}><color attach="background" args={["#e7ebe7"]}/><hemisphereLight args={["#f5f8ff", "#b6ad96", 2.2]}/><directionalLight position={[-5, 12, -7]} intensity={3} castShadow shadow-mapSize={[2048, 2048]} shadow-camera-left={-15} shadow-camera-right={15} shadow-camera-top={15} shadow-camera-bottom={-15} shadow-bias={-.0004}/><ArchitecturalModel design={design} style={style} furnished={furnished} cutaway={cutaway}/><ContactShadows position={[w / 2, -.24, d / 2]} scale={extent * 2} opacity={.38} blur={2} far={4} frames={1}/><OrbitControls makeDefault target={[w / 2, 0, d / 2]} minDistance={6} maxDistance={30} minPolarAngle={.05} maxPolarAngle={Math.PI / 2.15} enableDamping/></Canvas>}
