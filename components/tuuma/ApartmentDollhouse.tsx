...
export function ModelFallback({ apartment }: {
    apartment: Apartment;
}) { const d = designFor(apartment.id); const { text } = useLanguage(); const f20 = apartment.id === "F20"; return <div className="grid h-full min-h-80 place-items-center bg-[#f0f2ef] p-4 sm:p-6">{f20 ? <div className="max-w-5xl overflow-hidden rounded-2xl bg-white shadow-[0_16px_35px_rgba(18,49,76,.16)]"><img src="/art/kalliolinna-f20-cinematic-concept-v1.webp" alt={text({ fi: "F20:n alkuperäinen konseptirenderöinti", en: "Original F20 concept render", sv: "Original F20-konceptvisualisering" })} className="aspect-[3/2] w-full object-cover" /><div className="p-4 text-center text-sm leading-6 text-[#465f72]"><b className="text-[#173655]">{text({ fi: "F20 · konseptivisualisointi", en: "F20 · concept visualization", sv: "F20 · konceptvisualisering" })}</b><span className="block">{text({ fi: "WebGL ei ole käytettävissä tällä laitteella. Pohjapiirustus säilyy alla ja 3D avautuu tuetulla selaimella.", en: "WebGL is unavailable on this device. The floor plan remains below and 3D opens in a supported browser.", sv: "WebGL är inte tillgängligt på den här enheten. Planritningen finns nedan och 3D öppnas i en webbläsare som stöds." })}</span></div></div> : <><svg viewBox={`-500 -2300 ${d.width + 1000} ${d.depth + 3000}`} className="max-h-80 w-full" aria-label={`${apartment.id} floor plan`}><PlanGeometry design={d}/></svg><p className="mt-4 text-center text-sm text-[#465f72]">{text({ fi: "3D edellyttää WebGL-tukea. Tarkka pohjapiirustus on käytettävissä alla.", en: "3D requires WebGL. The vector floor plan remains available below.", sv: "3D kräver WebGL. Vektorplanen är tillgänglig nedan." })}</p></>}</div>; }
