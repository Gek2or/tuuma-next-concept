...
import { ArrowLeft, ArrowRight, Heart, Images, LayoutPanelTop, Box, Scan, Share2, Check, MapPin } from "lucide-react";
  ...
  return <div className="showcase-page">
    <div className="shell pt-7"><Link href="/kohteet" className="inline-flex min-h-11 items-center gap-2 text-sm font-bold"><ArrowLeft size={16}/>{copy("Asuntohakuun", "Back to home search", "Till bostadssökning")}</Link>
      ...
      <div className="grid items-start gap-7 xl:grid-cols-[minmax(0,1fr)_300px]">
        ...
        <aside className="showcase-details xl:sticky xl:top-24"><p className="text-sm font-semibold">{apartment.layoutLabel}</p><p className="mt-2 text-4xl font-extrabold tracking-tight">{apartment.size.toString().replace(".", ",")} <span className="text-lg">m²</span></p><p className="mt-4 text-2xl font-bold">{money}<span className="text-sm font-normal"> / {copy("kk", "month", "mån")}</span></p><p className="mt-1 text-xs text-[#535873]">{copy("Esimerkkivuokra · ei vuokratarjous", "Illustrative rent · not a rental offer", "Exempelhyra · inget hyreserbjudande")}</p><p className="mt-5 flex gap-2 text-sm leading-6"><MapPin size={18} className="mt-1 shrink-0"/>{apartment.address}</p><dl className="mt-5 space-y-3 border-y border-[#d8dae5] py-5 text-sm"><div className="flex justify-between"><dt>{copy("Tasot", "Levels", "Våningar")}</dt><dd className="font-bold">{apartment.levels}</dd></div><div className="flex justify-between"><dt>{copy("Ulkotila", "Outdoor space", "Uteplats")}</dt><dd className="font-bold">{copy("Oma terassi", "Private terrace", "Egen terrass")}</dd></div><div className="flex justify-between"><dt>{copy("Lemmikit", "Pets", "Husdjur")}</dt><dd className="font-bold">{copy("Tervetuloa", "Welcome", "Välkomna")}</dd></div></dl><Link href={`/hae?asunto=${apartment.id}`} className="showcase-primary mt-5">{copy("Kokeile hakemusta", "Try application", "Prova ansökan")}<ArrowRight size={18}/></Link><Link href={`/kustannukset?asunto=${apartment.id}`} className="mt-3 flex min-h-11 items-center justify-center text-center text-sm font-bold underline underline-offset-4">{copy("Laske kaikki asumiskulut", "Calculate total living costs", "Räkna alla boendekostnader")}</Link><Link href="/kohteet?compare=C09,E15,F20" className="flex min-h-11 items-center justify-center text-sm font-bold underline underline-offset-4">{copy("Vertaa kolmea kotia", "Compare three homes", "Jämför tre hem")}</Link><div className="mt-4 grid grid-cols-2 gap-2"><button onClick={favourite} aria-pressed={saved} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border bg-white text-sm font-semibold"><Heart size={17} fill={saved ? "currentColor" : "none"}/>{saved ? copy("Tallennettu", "Saved", "Sparad") : copy("Tallenna", "Save", "Spara")}</button><button onClick={() => void share()} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border bg-white text-sm font-semibold"><Share2 size={17}/>{copy("Jaa", "Share", "Dela")}</button></div><p role="status" className="mt-2 text-sm">{notice}</p></aside>
      </div>
      <section className="my-10 grid gap-5 border-t border-[#d8dae5] pt-9 md:grid-cols-3">{[
        { title: copy("Arjen tilat", "Everyday spaces", "Vardagens rum"), body: apartment.id === "C09" ? copy("Yksi taso, erillinen makuuhuone ja vaatehuone. Keittiöön, pesutilaan ja oleskeluun kuljetaan yhteisestä eteisestä.", "One level, a separate bedroom and storage room. The hall connects kitchen, bathroom and living spaces.", "Ett plan, separat sovrum och klädkammare. Hallen förbinder kök, badrum och vardagsrum.") : copy(`${apartment.rooms - 1} makuuhuonetta yläkerrassa. Yhteiset tilat ovat alhaalla. Portaan paikka ja aukko on sovitettu molempiin tasoihin.`, `${apartment.rooms - 1} bedrooms upstairs, shared living downstairs. Stairs and the opening align across both levels.`, `${apartment.rooms - 1} sovrum på övervåningen och gemensamma rum nere. Trappan och öppningen ligger i linje på båda våningarna.`) },
        { title: copy("Pinnat ja kalustus", "Finishes and furnishing", "Ytor och möblering"), body: copy("Tutki tammilattiaa, laatoitettuja märkätiloja ja kiintokalusteita. 3D:ssä voit vaihtaa sisustusta ja poistaa irtokalusteet.", "Explore oak flooring, tiled wet rooms and fixed fittings. In 3D, change the interior palette or remove movable furniture.", "Utforska ekgolv, kaklade våtrum och fast inredning. I 3D kan du byta stil eller ta bort lösa möbler.") },
        { title: copy("Hyrylä lähellä", "Hyrylä nearby", "Hyrylä nära"), body: copy("Tutustu alueen arkeen, luontoon ja palveluihin. Kohdekohtaiset matka-ajat ja palvelut tarkistetaan ennen oikean asunnon julkaisua.", "Explore local life, nature and services. Property-specific travel times and services are verified before publishing a real listing.", "Lär känna vardagen, naturen och servicen. Objektspecifika restider och tjänster kontrolleras före en riktig publicering.") },
      ].map(item => <article key={item.title}><Check size={22} className="text-[#3d4785]"/><h2 className="mt-3 text-xl font-extrabold">{item.title}</h2><p className="mt-3 text-base leading-7 text-[#535873]">{item.body}</p></article>)}</section>
    ...
    <ThreeHomeCards/>
    <div className="mobile-apply-cta fixed inset-x-3 z-40 flex items-center justify-between gap-3 rounded-2xl border border-[#d8dae5] bg-white p-3 shadow-lg xl:hidden"><span className="text-sm font-bold text-[#22264b]">{apartment.id} · {apartment.size} m²</span><Link href={`/hae?asunto=${apartment.id}`} className="showcase-primary px-5">{copy("Kokeile hakemusta", "Try application", "Prova ansökan")}<ArrowRight size={17}/></Link></div>
  </div>;
}
