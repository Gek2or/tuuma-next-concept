...
export default function RootLayout({children}:Readonly<{children:React.ReactNode}>){const schema={"@context":"https://schema.org","@type":"WebSite",name:"Tuuma Next — Digital Living Concept",inLanguage:"fi",description:"Demo of a modern digital housing service layer."};return <html lang="fi"><body><a href="#main" className="sr-only focus:not-sr-only focus:fixed focus:z-[100] focus:bg-white focus:p-4">Siirry sisältöön</a><PwaRegister/><SiteHeader/>{children}<SiteFooter/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/></body></html>}
